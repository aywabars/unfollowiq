...
module.exports = {
  ...
  async loginWithCredentials(username, password) {
    ...
    if (loginData.checkpoint_url || loginData.message === 'checkpoint_required') {
      ...
      try {
        // === USE MOBILE API FOR CHALLENGE ===
        // Convert checkpoint URL to mobile API path
        // checkpoint_url is like "/challenge/AgIW3HrY..." -> use as-is with i.instagram.com
        // Extract path from full URL if needed (checkpoint_url can be full URL like https://www.instagram.com/challenge/...)
        ...
        console.log('[CHALLENGE] Using mobile API path:', mobilePath);
        console.log('[CHALLENGE] Cookies:', allCookies.substring(0, 100));

        // Step 1: GET challenge info from mobile API (returns JSON)
        const challengeInfoRes = await rawRequest(
          ...
          null,
          allCookies,
          MOBILE_BASE  // Use i.instagram.com
        );

        console.log('[CHALLENGE] GET mobile API status:', challengeInfoRes.status);
        console.log('[CHALLENGE] GET mobile API data:', JSON.stringify(challengeInfoRes.data).substring(0, 500));
        ...
        // Step 2: POST choice=1 to mobile API to send verification code
        ...
        console.log('[CHALLENGE] POST choice=1 mobile API status:', sendCodeRes.status);
        console.log('[CHALLENGE] POST choice=1 mobile API data:', JSON.stringify(sendCodeRes.data).substring(0, 500));
        ...
        // Determine challenge type from mobile API response
        ...
        if (codeData) {
          ...
          // Mobile API also returns contact_point (masked phone/email)
        }

        // Check if we got JSON (mobile API success) vs HTML (fallback)
        const isJson = !codeData.raw;

        return {
          success: false,
          challengeRequired: true,
          checkpointUrl: mobilePath,  // Use mobile API path for verification too
          csrfToken: finalCsrf,
