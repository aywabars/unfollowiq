...
// Extract server revision from Instagram HTML for X-Instagram-AJAX header
function extractServerRevision(html) {
  if (!html) return null;
  const match = html.match(/"server_revision":(\d+)/);
...
module.exports = {
  ...
  async loginWithCredentials(username, password) {
    ...
    if (loginData.checkpoint_url || loginData.message === 'checkpoint_required') {
      ...
      try {
        // Step 1: GET the challenge page (returns HTML - we need to extract server_revision)
        ...
        console.log('[CHALLENGE] GET challenge page status:', challengeRes.status);

        // Extract the server revision (rollout_hash) from HTML for the AJAX header
        const challengeHtml = challengeRes.data.raw || '';
        const serverRevision = extractServerRevision(challengeHtml);
        console.log('[CHALLENGE] Extracted server_revision:', serverRevision);

        // Step 2: Request the verification code with proper AJAX headers
        // choice=1 = phone/WhatsApp, choice=0 = email
        ...
        // Build AJAX headers that tell Instagram to return JSON instead of HTML
        const ajaxHeaders = {
        ...
        if (serverRevision) {
          ajaxHeaders['X-Instagram-AJAX'] = serverRevision;
        }

        const sendCodeRes = await rawRequest(checkpointUrl, 'POST', ajaxHeaders, 'choice=1', challengeCookies);
        ...
        if (sendCodeRes.data) {
          if (sendCodeRes.data.step_name === 'verify_email') challengeType = 'email';
          else if (sendCodeRes.data.step_name === 'verify_code') challengeType = 'phone';
        }
        ...
        return {
          ...
          debugInfo: {
            challengeGetStatus: challengeRes.status,
            sendCodeStatus: sendCodeRes.status,
            sendCodeData: sendCodeRes.data,
            serverRevision,
            challengeGetDataPreview: typeof challengeRes.data === 'object' ? (challengeRes.data.raw ? challengeRes.data.raw.substring(0, 200) : JSON.stringify(challengeRes.data).substring(0, 200)) : String(challengeRes.data).substring(0, 200)
          }
        };
      } catch (challengeErr) {
  ...
  async verifyChallenge(checkpointUrl, code, csrfToken, cookies, username, serverRevision) {
    // Build AJAX headers for challenge verification too
    const headers = {
    ...
    if (serverRevision) {
      headers['X-Instagram-AJAX'] = serverRevision;
    }

    const body = `security_code=${encodeURIComponent(code)}`;
    const res = await rawRequest(checkpointUrl, 'POST', headers, body, cookies);
